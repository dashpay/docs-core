...
| →<br>`height`               | number (int)   | Optional<br>(0 or 1)    |  If the transaction has been included in a block on the local active block chain, this is the block height where the transaction was mined.  Otherwise, this is `-1`. Not shown for mempool transactions. |
